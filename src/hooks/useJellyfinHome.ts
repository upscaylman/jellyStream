...
function computeBadge(item: BaseItemDto): string | undefined {
  ...
  if (item.Type === 'Series' && (item.ChildCount ?? 0) > 1) {
    ...
    if (lastMedia) {
      const daysSince = (Date.now() - new Date(lastMedia).getTime()) / 86_400_000;
...
function toMovie(item: BaseItemDto, serverUrl: string) {
...
export function useJellyfinHome(): JellyfinHomeData {
  ...
  if (__DEV__) {
    if (latestMovies.isError) console.warn('[Home] latestMovies error:', latestMovies.error);
    if (latestSeries.isError) console.warn('[Home] latestSeries error:', latestSeries.error);
    if (trending.isError) console.warn('[Home] trending error:', trending.error);
    if (recentlyAdded.isError) console.warn('[Home] recentlyAdded error:', recentlyAdded.error);
    if (newlyAdded.isError) console.warn('[Home] newlyAdded error:', newlyAdded.error);
  }
  ...
  const featuredItem = allItems.find(
    (item) => item.BackdropImageTags?.length || item.ImageTags?.['Backdrop'],
  ) ?? allItems[0];

  if (featuredItem) {
    const backdropTag = featuredItem.BackdropImageTags?.[0] ?? featuredItem.ImageTags?.['Backdrop'];
    const primaryTag = featuredItem.ImageTags?.['Primary'];
    featured = {
      id: featuredItem.Id ?? '',
      title: featuredItem.Name ?? '',
      thumbnail: backdropTag
        ? getBackdropUrl(serverUrl, featuredItem.Id ?? '', 1280, 80, backdropTag)
        : primaryTag
          ? getImageUrl({ serverUrl, itemId: featuredItem.Id ?? '', maxWidth: 1280, quality: 80, tag: primaryTag })
          : '',
