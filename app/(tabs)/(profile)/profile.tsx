...
import { useFavoriteItems, useResumeItems } from '@/src/api/queries/useMediaQueries';
...
export default function ProfileScreen() {
    ...
    const getItemPoster = useCallback((item: BaseItemDto) => {
        const tag = item.ImageTags?.['Primary'];
        if (tag) {
            return getImageUrl({ serverUrl, itemId: item.Id ?? '', maxWidth: 200, quality: 80, tag });
        }
        return '';
    }, [serverUrl]);
        ...
        return (
            ...
            >
                {uri ? (
                    ...
                    <View style={[styles.likedShowImage, { backgroundColor: '#2a2a2a', justifyContent: 'center', alignItems: 'center' }]}>
    ...
    const renderFavorites = () => (
        <View style={styles.likedContent}>
            {isLoadingFav ? (
                <ActivityIndicator size="small" color="#E50914" style={{ padding: 20 }} />
                ...
                <Text style={{ color: '#666', padding: 16 }}>Aucun favori pour le moment</Text>
            )}
        </View>
    );
    ...
    const renderResumeList = () => (
        <View style={styles.likedContent}>
            {isLoadingResume ? (
                ...
                <Text style={{ color: '#666', padding: 16 }}>Rien en cours de lecture</Text>
            )}
        </View>
    );
