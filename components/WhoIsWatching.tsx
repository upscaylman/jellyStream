...
import { TouchableOpacity, StyleSheet, Dimensions, Image, StatusBar, LayoutRectangle, LayoutChangeEvent, Platform, findNodeHandle } from 'react-native';
...
export function WhoIsWatching({ onProfileSelect }: Props) {
    ...
    const profileSelectSound = useAudioPlayer(require('../assets/audio/profile-selected.mp3'));
    ...
    const handleProfileSelect = async (profile: Profile) => {
        try {
            profileSelectSound.seekTo(0);
            profileSelectSound.play();
    ...
    const handleProfileLayout = (profile: Profile, event: LayoutChangeEvent) => {
        const { x: lx, y: ly, width: lw, height: lh } = event.nativeEvent.layout;

        if (Platform.OS === 'web') {
            // Sur web, event.target.measure n'existe pas.
            // On utilise getBoundingClientRect via le DOM natif.
            const node = event.target as unknown as HTMLElement;
            if (node && typeof node.getBoundingClientRect === 'function') {
                const rect = node.getBoundingClientRect();
                const imageSize = rect.width * 0.8;
                setProfileLayouts(prev => ({
                    ...prev,
                    [profile.id]: {
                        x: rect.left + (rect.width - imageSize) / 2,
                        y: rect.top + (rect.width - imageSize) / 2,
                        width: imageSize,
                        height: imageSize,
                        originalX: rect.left,
                        originalY: rect.top,
            ...
            // Sur native, measure() est disponible
